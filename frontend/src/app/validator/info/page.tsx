        ...
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-8">
            <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-3xl sm:text-4xl font-bold mb-3">
              <span className="gradient-text">Validator Lookup</span>
            </motion.h1>
          ...
          <div className="card mb-6">
            <h2 className="text-lg font-semibold flex items-center gap-2 mb-4">
              <Server className="w-5 h-5 text-[#6366f1]" /> Validator Public Key
            </h2>
            <div className="flex gap-3">
              ...
              <button onClick={lookupValidator} disabled={loading} className="btn-primary flex items-center gap-2 shrink-0">
                ...
                <div className="card">
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-lg font-semibold flex items-center gap-2">
                      <Shield className="w-5 h-5 text-[#6366f1]" /> Validator Info
                    </h2>
                    {info.is_delinquent ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="p-3 rounded-lg bg-[var(--color-bg-input)] border border-[var(--color-border)] sm:col-span-2">
                      <p className="text-xs text-[var(--color-text-muted)] mb-1">Identity Pubkey</p>
                      <p className="text-sm font-mono text-[#14f195] break-all">{info.pubkey}</p>
                    </div>

                    {info.vote_pubkey && (
                      <div className="p-3 rounded-lg bg-[var(--color-bg-input)] border border-[var(--color-border)] sm:col-span-2">
                        <p className="text-xs text-[var(--color-text-muted)] mb-1">Vote Account</p>
                        <p className="text-sm font-mono text-[var(--color-text-secondary)] break-all">{info.vote_pubkey}</p>
                      </div>
                    )}

                    <div className="p-4 rounded-lg bg-[var(--color-bg-input)] border border-[var(--color-border)]">
                      <div className="flex items-center gap-2 mb-2"><Coins className="w-4 h-4 text-[#f59e0b]" /><p className="text-xs text-[var(--color-text-muted)]">Activated Stake</p></div>
                      <p className="text-lg font-bold">{formatSol(info.activated_stake)}</p>
                    </div>

                    <div className="p-4 rounded-lg bg-[var(--color-bg-input)] border border-[var(--color-border)]">
                      <div className="flex items-center gap-2 mb-2"><Hash className="w-4 h-4 text-[#6366f1]" /><p className="text-xs text-[var(--color-text-muted)]">Commission</p></div>
                      <p className="text-lg font-bold">{info.commission !== null ? `${info.commission}%` : "—"}</p>
                    </div>

                    {info.last_vote !== null && (
                      <div className="p-3 rounded-lg bg-[var(--color-bg-input)] border border-[var(--color-border)] sm:col-span-2">
                        <p className="text-xs text-[var(--color-text-muted)] mb-1">Last Vote Slot</p>
                        <p className="text-sm font-mono text-[var(--color-text-secondary)]">{info.last_vote?.toLocaleString()}</p>
                      </div>
                    )}
                  </div>
                </div>
